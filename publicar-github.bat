...
if not exist .git (
    ...
    echo [1/4] Repositorio git ya existe, sigo...
)

git remote get-url origin >nul 2>nul
if errorlevel 1 (
    echo [2/4] Conectando con GitHub...
    git remote add origin https://github.com/altagraciaavanza-create/sitioweb.git
) else (
    echo [2/4] El remoto "origin" ya esta configurado, sigo...
)

echo [3/4] Agregando cualquier cambio pendiente...
git add -A
git diff --cached --quiet
if errorlevel 1 (
    git commit -m "Actualizacion del sitio"
) else (
    echo No hay cambios nuevos, sigo con el push del commit existente...
)
